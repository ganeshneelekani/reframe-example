...
* [`src/reframe_example/`](src/reframe_example/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/reframe_example/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `reframe-example` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/reframe_example/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns reframe-example.example
  (:require [reframe-example.config :as config])
